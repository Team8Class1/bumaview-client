...
export function useToggleBookmarkMutation() {
  ...
  return useMutation({
    ...
    onMutate: async (interviewId: number) => {
      ...
      // 이전 데이터 백업
      const previousBookmarks = queryClient.getQueryData<AllInterviewDto[]>(
        bookmarkKeys.lists(),
      );
      ...
      if (previousBookmarks) {
        ...
        if (isCurrentlyBookmarked) {
          // 북마크 제거
          const newBookmarks = previousBookmarks.filter(
            (item) => item.interviewId !== interviewId,
          );
    ...
    onSuccess: (isBookmarked: boolean, interviewId: number) => {
      console.log(
        `🎉 북마크 토글 성공: interviewId=${interviewId}, isBookmarked=${isBookmarked}`,
      );

      // 서버 응답에 따라 캐시 직접 업데이트
      const currentBookmarks =
        queryClient.getQueryData<AllInterviewDto[]>(bookmarkKeys.lists()) || [];
      ...
      if (isBookmarked) {
        ...
        console.log(
          `🗑️ 북마크 목록에서 제거: interviewId=${interviewId}, 남은개수=${updatedBookmarks.length}`,
        );
      }
    ...
    onError: (error: any, interviewId: number, context: any) => {
      ...
      toast({
        ...
        title: "북마크 변경 실패",
        description:
          error?.message || "북마크 변경에 실패했습니다. 다시 시도해주세요.",
      });
    },
...
export function useBookmarkStatus(interviewId: number) {
  ...
  const isBookmarked =
    bookmarks?.some((item) => item.interviewId === interviewId) ?? false;

  console.log(
    `📊 북마크 상태 확인: interviewId=${interviewId}, isBookmarked=${isBookmarked}, 전체북마크수=${bookmarks?.length || 0}`,
  );
