...
export const interviewAPI = {
  ...
  search: (params?: InterviewFilterParams): Promise<InterviewListResponse> => {
    ...
    if (params?.myQuestions) searchParams.append('myQuestions', params.myQuestions.toString())

    const queryString = searchParams.toString()
    return api.get(`api/interview/search${queryString ? `?${queryString}` : ''}`).json()
  },
  ...
  // 단건 질문 다듬기
  trimSingle: (data: InterviewTrimSingleRequest): Promise<InterviewTrimSingleResponse> => {
    const queryParams = new URLSearchParams()
    queryParams.append('question', data.question)
    if (data.companyId) queryParams.append('companyId', data.companyId.toString())
    queryParams.append('questionAt', data.questionAt)

    return api.get(`api/interview/trim/single?${queryParams.toString()}`).json()
  },
