...
export const bookmarkAPI = {
  ...
  getAll: async (): Promise<AllInterviewDto[]> => {
    try {
      const response = await api
        .get("bookmark")
        .json<AllInterviewDto[] | BookmarkResponse>();
  ...
  toggle: async (interviewId: number): Promise<boolean> => {
    ...
    try {
      const response = await api
        .patch(`bookmark/${interviewId}`)
        .json<boolean | BookmarkToggleResponse>();
      ...
      console.error(
        `❌ 북마크 토글 API 실패 (interviewId: ${interviewId}):`,
        error,
      );
      throw error;
    }
